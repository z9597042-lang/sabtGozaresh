 // ===== API WRAPPER =====
      ...
      (function () {
        // تعریف آدرس‌های پایه برای هر سرویس
        const BASE_URL = "http://localhost:8080";
        const SERVICES = {
          // // reports:"/reports",ایجاد گزارش
          // // اپدیت با پوت /reports/{id}
          // // با دلت /reports/{id} برای دلت کردن گزارش
          // // برای گرفتن گزارش های خود یوزر/reports/myReports
          // // 
          // // /report/{id}  serch  ایدی رو از کاربر بگیره با متد GET
          // //  "/reports/newReport", // سرویس گزارشات     قدیمی
          // // users: "/api/users", // سرویس کاربران
          // // products: "/api/products", // سرویس محصولات
          // departments: BASE_URL + "/myDepartments",
          // // "/api/departments", // سرویس دپارتمان‌ها
          // // auth: BASE_URL + "/auth",
          // // "/login", // سرویس احراز هویت
          // testapi: BASE_URL + "/reports/check",
          // //  "/reports/check", //سرویس تست api
        };
        ...
        window.API = {
          ...
          reports: {
            getAll: () => window.API.get("reports", ""),
            getOne: (id) => window.API.get("reports", `/${id}`),
            create: (data) => window.API.post("reports", "", data),
            update: (id, data) => window.API.put("reports", `/${id}`, data),
            delete: (id) => window.API.delete("reports", `/${id}`),
          },

          // users: {
          //   getAll: () => window.API.get("users", ""),
          //   getOne: (id) => window.API.get("users", `/${id}`),
          ...
          // products: {
          //   getAll: () => window.API.get("products", ""),
          //   getOne: (id) => window.API.get("products", `/${id}`),
          //   create: (data) => window.API.post("products", "", data),
          //   update: (id, data) => window.API.put("products", `/${id}`, data),
          //   delete: (id) => window.API.delete("products", `/${id}`),
          // },
          ...
          // testapi: {
          //   check: () => window.API.get("testapi", "/status"), // ← GET /api/test/status
          //   ping: () => window.API.get("testapi", "/ping"), // ← GET /api/test/ping
          //   info: () => window.API.get("testapi", ""), // ← GET /api/test
          // },
        };
      })();
      // ===== حالت شب/روز =====
        ...
        Chart.register({
          ...
          afterDatasetsDraw(chart) {
            ...
            chart.data.datasets.forEach((dataset, datasetIndex) => {
              ...
              const points = meta.data.map((bar) =>
                bar.getProps(["x", "y"], false),
              );
      ...
      (function () {
        ...
        function syncRowInputsCount() {
          ...
          const rowGroups = document.querySelectorAll(
            "#rowsContainer .row-item",
          );
        ...
        async function deleteReport(id) {
          try {
            ...
            await fetchReports();
            // if (window._lastSelectedId === id) {
            //   detailContainer.innerHTML = `<span style="color: var(--text-muted);">گزارش حذف شد</span>`;
            //   window._lastSelectedId = null;
            // }
            return true;
          } catch (e) {
        ...
        // ===== 👇 اینجا (بعد از خط ~۸۰۰) کد تست رو اضافه کن =====
        ...
        // ===== 👇 اینجا (بعد از کد تست) رویداد دکمه رو اضافه کن =====
        ...
        // ===== CREATE CHARTS (نسخه با تشخیص عمودی/افقی) =====
          ...
          if (barCtx && cols.length > 0 && rows.length > 0) {
            ...
            cols.forEach((col, colIndex) => {
              ...
              datasets.push({
                label: col,
                data: data,
                // ===== ویژگی‌های میله (به تعداد ردیف‌ها، برای هر ستون) =====
                type: "bar",
                ...
                // ===== خط همان ستون، دقیقاً روی نوک هر میله (توسط barTopLinePlugin رسم می‌شود) =====
                topLine: {
                  color: color.main,
                  width: 3,
                  pointRadius: 5,
            ...
            // ===== تنظیمات محورها (یکسان در هر دو جهت؛ فقط محور مقدار/دسته‌بندی جابه‌جا می‌شود) =====
            const isDark =
            ...
            // ===== ارتفاع نمودار متناسب با تعداد میله‌ها (بزرگ‌تر، خصوصاً در حالت عمودی) =====
        ...
        async function saveReport(dto, isUpdate = false, id = null) {
          try {
            ...
            const data = await res.json();
            showToast(
              isUpdate ? "گزارش با موفقیت ثبت شد" : "گزارش بروزرسانی شد",
            );
            await fetchReports();
        ...
        document.addEventListener("input", function (e) {
          if (
            e.target.classList.contains("col-input") &&
            e.target.value.trim()
          ) {
        ...
        form.addEventListener("submit", async function (e) {
          ...
          if (!rowValid) {
            showToast(
              "تعداد مقادیر هر ردیف باید برابر تعداد ستون‌ها باشد",
              true,
            );
            return;
          }
      ...
      //   showToast("جلسه شما منقضی شد. لطفاً دوباره وارد شوید.", true);
      //   // اگه صفحه لاگین داری، بهش هدایت کن
      ...
      (function () {
        ...
        function startSessionTimer() {
          ...
          // ۵ دقیقه قبل از اتمام، هشدار بده
          // warningTimer = setTimeout(() => {
          //   showToastMessage("⚠️ جلسه شما در ۵ دقیقه دیگر منقضی می‌شود. لطفاً فعالیت کنید.", false);
